...
bool BakerByGlobalState::OnMessage(Baker* baker, const Telegram& msg) { 
    ...
    case mesg_HEY:
    {
        if (baker->Location() == Bakery) {
            Dispatch->DispatchMessage(SEND_MSG_IMMEDIATELY, baker->ID(), ent_PasserBy, mesg_WELCOME, (void*)NO_ADDITIONAL_INFO);
            baker->GetFSM()->ChangeState(HiWithPasserBy::Instance());
        }
        else {
...
{
    SetTextColor(FOREGROUND_RED | FOREGROUND_INTENSITY);
    cout << "\n" << GetNameOfEntity(pBaker->ID()) << ": "<< "Leaving Oven";
}
...
{
    SetTextColor(FOREGROUND_RED | FOREGROUND_INTENSITY);
    cout << "\n" << GetNameOfEntity(pBaker->ID()) << ": " << " : HI PasserBy Eat this COOKIES";
}
