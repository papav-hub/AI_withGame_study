...
void Baking_Oven::Execute(Baker* pBaker)
{
    ...
    {
        pBaker->ChangeState(Selling_Bakery::Instance());
    }

    else if (pBaker->noIngredient())
    {
        pBaker->ChangeState(Buy_ShoppingMall::Instance());
    }

    else if (pBaker->checkHealth())
    {
        pBaker->ChangeState(Rest_Home::Instance());
    }
}
...
void Selling_Bakery::Execute(Baker* pBaker)
{
    ...
    if (pBaker->displayEmpty())
    {
        ...
        pBaker->ChangeState(Baking_Oven::Instance());
    }
}
